...
- Execute `cd ios && pod install && cd ..` to install iOS dependencies
- Execute `yarn ios` to start the application on iOS or `yarn android` to start the application on android
...
- Execute o comando `cd ios && pod install && cd ..` para instalar as dependencias do iOS
- Executar o comando `yarn ios` para iniciar a aplicação no ios ou `yarn android` para iniciar a aplicação no android



## Rocketseat **[Desafio11](https://github.com/Rocketseat/bootcamp-gostack-desafios/blob/master/desafio-react-native-delivery/README.md)**
